...
for category in [[train_app_path, test_app_path, gbdt_train_app_path, gbdt_test_app_path],
    ...
    n_estimator = 200
    # 调用GBDT分类模型。
    gd = GradientBoostingClassifier(n_estimators=n_estimator,max_depth=3)
    ...
    train_features = gd.apply(train_data_gbdt_x)
    train_features = train_features[:, :, 0].astype(int)
    train_df = pd.DataFrame(train_features)
    train_df['click'] = train_data_gbdt_y
    train_df.to_csv(category[2], index=False)

    test_features = gd.apply(test_data_gbdt_x)
    test_features = test_features[:, :, 0].astype(int)
